...
double aggKp=750, aggKi=2000, aggKd=0;
...
std_msgs::Int64MultiArray wheel_rpm;
std_msgs::Int64MultiArray enc_ticks;
std_msgs::Float64MultiArray vel_wheels;
std_msgs::Float64MultiArray setpoint;
std_msgs::Float64MultiArray output_pwm;

ros::Publisher set_point_pub("setpoint", &setpoint);
...
ros::Publisher wheel_rpm_pub("wheel_rpm", &wheel_rpm);
...
void setup() {
  ...
  myPID_fl.SetOutputLimits(-150, 150);
  myPID_fr.SetOutputLimits(-150, 150);
  myPID_bl.SetOutputLimits(-150, 150);
  myPID_br.SetOutputLimits(-150, 150);
  ...
  // setpoint array initialization
  char dim2_label[] = "setpoint";
  setpoint.layout.dim = (std_msgs::MultiArrayDimension *) malloc(sizeof(std_msgs::MultiArrayDimension) * 2);
  setpoint.layout.dim[0].label = dim2_label;
  setpoint.layout.dim[0].size = 4;
  setpoint.layout.dim[0].stride = 1*4;
  setpoint.data = (float *)malloc(sizeof(float)*4);
  setpoint.layout.dim_length = 0;
  setpoint.data_length = 4;
  
  //output_pwm array initialization
  char dim3_label[] = "output_pwm";
  output_pwm.layout.dim = (std_msgs::MultiArrayDimension *) malloc(sizeof(std_msgs::MultiArrayDimension) * 2);
  output_pwm.layout.dim[0].label = dim3_label;
  output_pwm.layout.dim[0].size = 4;
  output_pwm.layout.dim[0].stride = 1*4;
  output_pwm.data = (float *)malloc(sizeof(float)*4);
  output_pwm.layout.dim_length = 0;
  output_pwm.data_length = 4;

  //rpm array initialiazation
  char dim4_label[] = "wheel_rpm";
  wheel_rpm.layout.dim = (std_msgs::MultiArrayDimension *) malloc(sizeof(std_msgs::MultiArrayDimension) * 2);
  wheel_rpm.layout.dim[0].label = dim4_label;
  wheel_rpm.layout.dim[0].size = 4;
  wheel_rpm.layout.dim[0].stride = 1*4;
  wheel_rpm.data = (long long int *)malloc(sizeof(long long int)*4);
  wheel_rpm.layout.dim_length = 0;
  wheel_rpm.data_length = 4;
  
  nh.advertise(enc_ticks_pub);
  nh.advertise(wheel_rpm_pub);
  nh.advertise(vel_pub);
  nh.advertise(set_point_pub);
...
void loop() {
 ...
 // calculate time and current velocity
  ...
  float rpm_fl = ((float(ct1 - old_ct1) / ticks_per_meter) / ((now - prev))) * 60000;
  float rpm_fr = ((float(ct2 - old_ct2) / ticks_per_meter) / ((now - prev))) *60000;
  float rpm_bl = ((float(ct3 - old_ct3) / ticks_per_meter) / ((now - prev))) *60000;
  float rpm_br = ((float(ct4 - old_ct4) / ticks_per_meter) / ((now - prev))) *60000;

  wheel_rpm.data[0] = rpm_fl;
  wheel_rpm.data[1] = rpm_fr;
  wheel_rpm.data[2] = rpm_bl;
  wheel_rpm.data[3] = rpm_br;
  ...
  //publishing setpoint value
  setpoint.data[0] = Setpoint_fl;
  setpoint.data[1] = Setpoint_fr;
  setpoint.data[2] = Setpoint_bl;
  setpoint.data[3] = Setpoint_br;
  ...
  //Output PWM 
  output_pwm.data[0] = Output_fl;
  output_pwm.data[1] = Output_fr;
  output_pwm.data[2] = Output_bl;
  output_pwm.data[3] = Output_br;
